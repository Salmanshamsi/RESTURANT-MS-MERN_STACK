...
const Offers = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      const { image, date, _id, previousFile } = values;
      ...
      if (!isEditMode) {
        ...
        try {
          ...
          const updateOffer = await updateApiMethod(
            `/offer/updateOffer`,
            formData
          );
          if (updateOffer?.status == 200) {
            ...
            setIsLoad(!isLoad);
            setIsEditMode(false);
            toast.success(`${updateOffer?.data?.message}`, {
              position: "top-right",
              autoClose: 1000,
              hideProgressBar: false,
              closeOnClick: false,
  ...
  const handleEdit = async (e) => {
    setIsEditMode(true);
    setIsModalOpen(true);
    console.log("e", e?.previousFile);
    formik.setFieldValue("_id", e?._id);
    formik.setFieldValue("date", e?.expiryDate);
    formik.setFieldValue("image", e?.previousFile);
